...
const inventory = require("../model/user");
...
async function signUpService(body) {
  if (!body.email || !body.password) {
    ...
    await db.collection("User").doc(body.email).set(newCredentials);
    return {
      result: 0,
...
async function loginService(body) {
  if (!body.email || !body.password) {
    ...
    const userRef = await db.collection("User").doc(body.email).get();
    if (!userRef.exists) {
    ...
    return {
      result: 0,
      token: jwt.sign({ userId: userData.email }, process.env.JWT_SECRET, {
        expiresIn: process.env.JWT_LIFETIME,
      }),
    };
  }
